...
def _collect_reads(ir):
    ...
    Mirrors ``irvm._run_ir`` frame-entry snapshot semantics (guards included), so
    any address absent from the set is never consulted across playback.
    """
...
def compress(ir):
    ...
    The explicit per-frame trace is replaced by guard dispatch: an interned guard
    DAG, a ``guard-vector -> program`` table for derivable frames, and an RLE
    residual trace for the frames whose vector is ambiguous.
    """
    ...
    gpool, gindex = [], {}
    guard_roots = [_intern(g, gpool, gindex) for g in ir.get("guards", [])]
    ...
    return {
        ...
        "guard_table": sorted([v, pi] for v, pi in dispatch["table"].items()),
...
def decompress(comp):
    ...
    The trace is regenerated by running the guard dispatch (guard DAG + table +
    residual) over the reconstructed IR, proving guard selection reproduces it.
    """
    pool, memo = comp["pool"], {}
    programs = [
        {
    ...
    gp, roots = irvm.build_guard_pool(guards)
    residual = []
...
def count_tokens(comp):
    ...
    guard_table = len(comp["guard_table"])
