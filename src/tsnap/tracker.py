...
def read_freqtable(base_lo, base_hi, stride, mem, span=128):
    ...
    """
    return [
        mem[(base_lo + stride * i) & 0xFFFF] | (mem[(base_hi + stride * i) & 0xFFFF] << 8)
        for i in range(span)
    ]
...
def recover_tuning(path, song=0, frames=300):
    ...
    clock = NTSC_CLOCK if R.discover_cadence(path, song)["clock"] == "NTSC" else PAL_CLOCK
    voices = []
    for v in range(3):
...
def print_tuning(name, t):
    ...
    dv = ", ".join(f"v{i}:{v['detune']:+.1f}c" for i, v in enumerate(t["voices"]) if v is not None)
...
def capture_trace(path, song, note_cells, frames, watch=(), sel_cells=(None, None, None)):
    ...
    smc = R.smc_operands(path, song, min(frames, 512))
    ...
    for v in range(3):
        ...
        for f, st in enumerate(trace[v]):
            ...
            if g and not pg:
                onsets.append(f)
            pg = g
    gaps = [b - a for a, b in zip(sorted(set(onsets)), sorted(set(onsets))[1:]) if b > a]
    if not gaps:
        return 4
    g = reduce(gcd, gaps)
...
def _detect_loop(order):
    for start in range(len(order)):
        ...
        for period in range(1, len(rem) // 2 + 1):
            if len(rem) % period == 0 and all(rem[i] == rem[i % period] for i in range(len(rem))):
                return start, period
    return 0, len(order)
...
def pretty_resolved(e, cm):
    ...
    if e[0] == "op":
        return R.fmt(e[1], [pretty_resolved(k, cm) for k in e[2]])
    if e[0] == "mem":
        r = _index_read(e[1])
        if r:
            ...
            if tab is not None:
                ...
                fld = _field_names(tab).get(base - tab["base"], f"+{base - tab['base']}")
                return f"{nm}[{idx}].{fld}"
...
def cell_map(tables):
    ...
    return {
        "t": {(tab["stride"], c): tab for tab in tables.values() for c in tab["cells"].values()},
        "r": {
...
def resolve_tables(t):
        ...
        for r in range(7):
            ...
            for gen in t["variants"].get(addr, {}):
                ...
                for base, stride, cell, off, _sz in got:
                    grp = per[(stride, cell)]
                    grp["offs"].add(off)
                    grp["tbase"] = base if grp["tbase"] is None else min(grp["tbase"], base)
    ...
    for tab in tables.values():
        tab["is_pitch"] = any(r in (0, 1) for regs in tab["fields"].values() for _v, r in regs)
    return tables
...
def instr_table(tables, kinds):
    ...
    for tab in tables.values():
        ...
        if all(kinds.get(c) == "selector" for c in tab["cells"].values()):
            if len(tab["fields"]) > best_fields:
                best, best_fields = tab, len(tab["fields"])
    return best
...
def _field_names(tab):
    raw = {
        off: "/".join(sorted({_REGNAME[r] for _, r in regs})) for off, regs in tab["fields"].items()
    }
...
def _monotonic(d):
    return all(x <= y for x, y in zip(d, d[1:])) or all(x >= y for x, y in zip(d, d[1:]))
...
def classify_mod(devs):
    ...
    cyc = _cycle(semis)
    if len(offs) > 1 and quant < 20 and max(abs(o) for o in offs) <= 24 and len(cyc) <= 8:
        return ("arp", cyc)
...
def _emit_tables(tables, kinds, itab, mat):
    ...
    for _k, tab in sorted(tables.items()):
        ...
        print(f"  {kind}{star} @${tab['base']:04X} stride {tab['stride']} " f"index[{idx}]{offs}")
        fn = _field_names(tab)
