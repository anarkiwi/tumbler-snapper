...
import numpy as np
from conftest import COMMANDO, requires_commando, requires_vm
...
def test_trace_reconstructs_the_oracle_grid():
    ...
    frames = trace.trace_sid(COMMANDO, n)
    ...
    for f in range(n):
        for reg, val in trace.sid_stores(frames[f]):
            row[reg] = val
        grid[f] = row
    assert np.array_equal(sidreg.latch(grid), grid_from_sid(COMMANDO, n))
