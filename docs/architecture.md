...
| **< 1 token/frame** | on fixtures — 29/32 under 1.0 at full horizon (walk rung, lossless, debt 0), but walk `cfg` grows on the cfg-dominated tail. **Schedule-interpreter measurement** (`tools/seq_schedule_probe.py`): the machine-order cursor-canonicalized model is byte-exact and its frame-entry residual is **0** on all four cfg-dominated witnesses (collisions resolve — the `~99.6%` was the frozen frame-entry guard vector); measured seq tokens **bounded < 1.0 on 3/4** (Sc00ter 0.084, Old_Times 0.684, Take_Off 0.792 proj), **only Vacuole grows** (1.775) via its **packed-row decoder loop** (`$16CD…$176A` branch on decoded byte `$96`). Closing Vacuole needs the decoder re-execution (§2.i), not a token gate ([`seq-replay-rung.md`](seq-replay-rung.md), [`gap-audit.md`](gap-audit.md), [`tokens.md`](tokens.md)) |
